...
| `StaffDedicatedInterfaceCoverageTest` | Toute capacite cochee produit une section sur `/staff/{slug}` : la barre complete d'un type qui porte les vingt et une, **et le filet qui verifie qu'en decocher une change toujours la page**. Puis la saisie reelle depuis un poste dedie — consultation, imagerie, allergie et ordonnance atterrissent dans le DME, signees par leur auteur, **meme sans droit d'ouvrir le dossier complet** ; rendez-vous signe `staff_member_id` et non un medecin invente ; enregistrement hors capacite refuse cote serveur ; prescription de soins hors de portee du personnel generique. |
